  ...
  module TimeSeries
    ...
    class Query
      ...
      def initialize(config = {})
        ...
        # Create 'm' string
        @metrics.each do |m|
          metric_arr << parse_metric_string(m)
        end

        @config[:m] = metric_arr
        @format = @config.delete(:format).to_s || 'json'
        @type = @format == 'json' ? 'ascii' : @format
      end
      ...
      def validate_metrics(config, requirements)
        requirements.each do |req|
          next if config.keys.include?(req.to_sym) || config.keys.include?(req.to_s)
      ...
      def check_metrics(metrics)
        metrics.each do |h|
          fail(ArgumentError, "Expected a Hash - got a #{h.class}: '#{h}'") unless h.is_a? Hash
          %w(aggregator metric).each do |mtag|
            next if h.key?(mtag.to_sym)
            fail(ArgumentError, 'Aggregator and metric label must be present for query to run.')
          end
        end
      end

      def parse_metric_string(m)
        str = ''
        tag_params = []
        ds = m[:downsample]
      ...
      def build_request(params = {})
        ...
        params.each_pair do |k, v|
          if v.respond_to? :each
            v.each { |subv| query << "#{k.to_s.strip}=#{subv.to_s.strip}" }
          else
